...
def preprocess_image(image_path: str) -> cv2.Mat | Any:
...
def perform_ocr(image_path: str) -> str:
    ...
    text: str = pytesseract.image_to_string(processed, lang='eng')
    return text
...
def ocr_and_save(images: List[str], save_folder: str) -> Dict[str, str]:
...
def select_file_or_folder() -> List[str]:
  
    root = Tk()
    root.withdraw()

    choice: bool = messagebox.askyesno(
        "Select Mode",
        "Do you want to select a folder of images?\nYes = Folder, No = Single File",
    )
...
def select_save_folder() -> str | None:
...
if __name__ == "__main__":
    ...
    messagebox.showinfo(
        "OCR Completed",
        f"OCR finished for {len(images)} image(s).\nResults saved in {save_folder}."
    )
